...
def generate_html_report(days=90, output_path=None):
    ...
    # Enhanced: track daily data within each week
    ...
    for sgv, _, ds, _ in rows:
        try:
            ...
            weekly_daily_data[week_start][day_of_week].append(sgv)
        except (ValueError, TypeError):
            pass
    ...
    for idx, week_start in enumerate(sorted(weekly_data.keys())):
        values = weekly_data[week_start]
        if values:
            in_r = sum(1 for v in values if t["target_low"] <= v <= t["target_high"])
            week_tir = round(in_r / len(values) * 100, 1)
            
            # Calculate daily TIR for sparkline
            daily_tir = []
            best_day_name = None
            best_day_tir = -1
            
            for day_idx in range(7):
                day_values = weekly_daily_data[week_start].get(day_idx, [])
                if day_values:
                    day_in_r = sum(1 for v in day_values if t["target_low"] <= v <= t["target_high"])
                    day_tir_pct = round(day_in_r / len(day_values) * 100, 1)
                    daily_tir.append(day_tir_pct)
                    
                    # Track best day
                    if day_tir_pct > best_day_tir:
                        best_day_tir = day_tir_pct
                        best_day_name = day_names[day_idx]
                else:
                    daily_tir.append(None)
            
            # Calculate week-over-week trend
            tir_change = None
            if idx > 0 and len(weekly_stats) > 0:
                prev_tir = weekly_stats[-1]["tir"]
                tir_change = round(week_tir - prev_tir, 1)
            ...
            weekly_stats.append({
                ...
                "best_day_tir": best_day_tir if best_day_tir >= 0 else None,
...
<body>
    <div class="container">
        ...
        <div class="chart-section">
            ...
            <p style="color: var(--text-secondary); margin-bottom: 15px; font-size: 0.9rem;">
                Track your weekly progress with TIR trends and daily patterns.
            </p>
            <div class="chart-container">
                <canvas id="weeklyChart"></canvas>
            </div>
            <div id="weeklySummaryText" style="margin-top: 15px; padding: 15px; background: var(--bg-card); border-radius: 8px; font-size: 0.9rem;">
                <!-- Generated by JavaScript -->
            </div>
        </div>
    ...
    <script>
        ...
        function updateAllCharts(readings) {
            ...
            // Update weekly
            weeklyStats = weeklyData; // Update the global weeklyStats reference
            ...
            weeklyChart.update();
            updateWeeklySummaryText(); // Update the summary text
        ...
        let weeklyStats = %(weekly_stats_json)s; // Use 'let' to allow updates
        ...
        // Weekly Chart with Enhanced Context
                    ...
                    tooltip: {
                        callbacks: {
                            afterBody: function(context) {
                                if (context[0].datasetIndex === 1) { // TIR bar
                                    const idx = context[0].dataIndex;
                                    const week = weeklyStats[idx];
                                    const lines = [];
                                    ...
                                    if (week.daily_tir && week.daily_tir.length > 0) {
                                        ...
                                        if (week.daily_tir.some(v => v !== null)) {
                                            ...
                                            lines.push(`Daily: ${sparkline}`);
                                            lines.push('       M T W T F S S');
                                        }
                                    }
        ...
        function updateWeeklySummaryText() {
            const container = document.getElementById('weeklySummaryText');
            if (!weeklyStats || weeklyStats.length === 0) {
                container.innerHTML = '<p style="color: var(--text-secondary);">No weekly data available.</p>';
                return;
            }
            ...
            weeklyStats.forEach((week, idx) => {
                ...
                const bestDayText = week.best_day && week.best_day_tir !== null
                    ? `${week.best_day}: ${week.best_day_tir}%% TIR`
                    : 'No data';
                
                html += `
                    <div style="padding: 10px; background: var(--bg-secondary); border-radius: 6px; border-left: 3px solid ${week.tir >= 70 ? colors.inRange : week.tir >= 50 ? colors.low : colors.veryLow};">
                        <div style="font-weight: 600; margin-bottom: 5px;">Week of ${week.week}</div>
                        <div style="font-size: 0.85rem; color: var(--text-secondary);">
                            <div>TIR: ${week.tir}%%</div>
                            <div>Trend: ${trendText}</div>
                            <div>Best: ${bestDayText}</div>
                        </div>
                    </div>
                `;
            });
